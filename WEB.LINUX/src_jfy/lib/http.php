 ...
 * @return bool|string false表示不完整的HTTP请求继续等下一个包;-1表示出错;非空字符串则为完整的HTTP请求
 */
function http_input($fd, $http_string)
{
    ...
    if ( strpos($http_string, "POST") !== false )
    {
        ...
        if(preg_match("/\r\nContent-Length: ?(\d*)\r\n/", $http_string, $match))
        {
            ...
            if ( $remain_length > 0 ) return false;
        }
    }
...
/**
 * 解析http协议，返回array('path_api','query_string')
 * @param string $http_string
 * @return array('path_api','query_string')
 */
function http_start_simple($http_string)
{
    // 将header分割成数组
    $header_data = explode("\r\n", $http_string, 1);

    list($request_method, $request_uri, $server_protocol) = explode(' ', $header_data[0]);
    $url_info = parse_url($request_uri);
    $path_info = pathinfo($url_info['path']);
    $result['path_api'] = $path_info['filename'];
    
    if($request_method == 'POST')
    {
        // 获取POST数据
        $tmp = explode("\r\n\r\n", $http_string, 2);
        $result['query_string'] = $tmp[1];
    } else {        
        $result['query_string'] = $url_info['query'];
    }
